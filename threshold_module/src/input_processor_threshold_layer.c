...
struct threshold_layer_config { uint32_t require_prior_idle_ms; };
...
static int threshold_layer_handle_event(const struct device *dev,
    ...
    const uint8_t layer = param1;
    const uint32_t timeout_ms = param2;
    const uint32_t threshold = param3;
    ...
    if (cfg->require_prior_idle_ms > 0 &&
        zmk_activity_get_last_activity() + cfg->require_prior_idle_ms > k_uptime_get()) {
        data->accumulated = 0;
        return ZMK_INPUT_PROC_CONTINUE;
    }
    ...
    if ((uint32_t)data->accumulated >= threshold) {
        data->accumulated = 0;
